...
const Login = () => {
  const [email, setEmail] = useState(""); // Maps to the "identifier" required by our backend
  const [password, setPassword] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [isLoading, setIsLoading] = useState(false);

  // NEW: State handlers to surface real database validation errors to user interface
  ...
  const handleSubmit = async (e) => {
    ...
    const loginPayload = {
      identifier: email.trim(), // Backend supports login via either email OR phone matching
      password: password,
    };
    ...
    try {
      ...
      // 1. Persist the generated JWT token to localStorage for authenticated requests
      localStorage.setItem("token", response.data.token);

      // 2. Cache key user parameters safely on the client level
      localStorage.setItem("user", JSON.stringify(response.data.user));

      // 3. Smart routing based on role metadata returned from our updated schema login token
      if (response.data.user.role === "admin") {
        navigate("/admin/dashboard");
      } else {
        navigate("/"); // Send customers back to store catalog grid view home page
      }
    } catch (error) {
      console.error("Authentication handshake error:", error);
      // Grabs our clean message generated by our Global Error Handling Middleware
      setErrorMessage(
        error.response?.data?.message ||
          "Invalid credentials or network connectivity dropout.",
      );
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen w-full bg-slate-100 flex flex-col justify-between items-center py-6 px-4 overflow-y-auto">
      ...
      <div className="w-full max-w-md bg-white border border-slate-200/80 rounded-sm shadow-xl shadow-slate-200/50 my-auto overflow-hidden shrink-0">
        <div className="p-6 sm:p-8">
          ...
          {/* NEW: Operational Warning Box displaying real server rejections */}
          {errorMessage && (
